...
for i in range(5):
    ...
    for j in range(4):
            ...
            for length in len_series:
                    ...
                    model = Sequential()
                    model.add(Conv1D(filters=100, kernel_size=3, activation=LeakyReLU(0.5), input_shape=(length, num_features)))
                    model.add(Dropout(0.8))
                    model.add(Conv1D(filters=100, kernel_size=3, activation=LeakyReLU(0.5)))
                    ...
                    # Compile the model
                    model.compile(optimizer=SGD(lr=0.00001, momentum=0.8), loss='binary_crossentropy',
